...
//                if ($os->getAgrupaLiquidacion() == 1) {
//                    $Plan = managerPlan::obtenerPlanPorId($_POST['plan0']);
//                    $farmacia = managerFarmacias::obtenerFarmaciaPorIdObj($userAuth->Farmacia->getIdFarmacia());
//                    //variables a utilizar en la vista preliminar
//                    $last = $farmacia->getContadorBarra() + 1;
//                    $codigoBarra = managerFactura::generarCodigoBarra($userAuth->Farmacia->getIdFarmacia(), $_POST['obraSocial'], $_POST[$plan0], $_SESSION['quincena'], $_SESSION['mes'], $_SESSION['anio'], $last);
//                    for ($i = 0; $i < $cantPlanes; $i++) {
//                        $sumImporte += $_POST[$importe . $i];
//                        $sumCargo += $_POST[$cargo . $i];
//                        $sumNeto += $_POST[$neto . $i];
//                        $cantRecetas += $_POST[$cantidad . $i];
//                        $listadoPlanes[] = managerPlan::obtenerPlanPorId($_POST[$plan . $i]);
//                    }
...
//                    $caratula->setFolioDesde($f1);
//                    $caratula->setFolioHasta($f2);
//                    $caratula->setPlan($Plan);
//                    $caratula->setOperador(strtoupper($userAuth->getUsuario()));
//                    $caratula->setCodigoBarra($codigoBarra);
//                    $caratula->setArancel($sumImporte);
//                    $caratula->setArancelOS($sumCargo);
//                    $caratula->setImporteBonificacion($sumNeto);
...
//                } else {
    ...
    if (!isset($error) || (isset($error) && $error)) {
        ?><div class="subtit2">Agregar</div>
            ...
            <form id="carAdd" name="carAdd" method="post" class="ajax"  action="caratulaSumaria.php" >
